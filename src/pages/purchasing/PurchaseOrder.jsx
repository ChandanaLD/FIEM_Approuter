...
export default function PurchaseOrder() {
  ...
  useEffect(() => {
    let cancelled = false
    PurchaseOrderApi.listAgreements({ search: searchQuery, plants: selectedPlants })
  ...
  useEffect(() => {
    ...
    PurchaseOrderApi.getAgreement(selectedAgreementId)
  ...
  const handleConfirm = async () => {
    if (!agreement) return
    try {
      await PurchaseOrderApi.confirmAgreement(agreement.id)
    } catch (err) {
      console.error(err)
    }
  }
